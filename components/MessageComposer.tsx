...
const MessageComposer: React.FC<MessageComposerProps> = ({ contacts }) => {
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    console.log('Media:', mediaFile ? { name: mediaFile.name, type: mediaFile.type } : 'None');
    console.log(`Sending to ${contacts.length} contacts:`, contacts.map(c => c.phone));
    console.log('-------------------------------');

    setTimeout(() => {
      setIsSending(false);
      setSendStatus({ type: 'success', message: `Message successfully sent to ${contacts.length} contacts.` });
      setHeading('');
      setContent('');
      setMediaFile(null);
      setMediaPreview(null);
